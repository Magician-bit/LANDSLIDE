...
import { X, Play, RotateCcw, CloudRain, Droplets, ShieldAlert } from 'lucide-react';
...
export default function ScenarioSimulator({ intel, onClose }: { intel: any; onClose: () => void }) {
  ...
  const handleActivate = (type: Scenario['type'], updates: Partial<Scenario>) => {
    if (typeof intel?.setScenario === 'function') {
      intel.setScenario({
        ...intel.scenario,
        active: true,
        type,
  ...
  const handleReset = () => {
    if (typeof intel?.resetScenario === 'function') {
      intel.resetScenario();
    }
  };
    ...
    <div className="flex flex-col h-full bg-slate-900 text-slate-300">
      <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-950/60">
        <div>
          ...
          <p className="text-xs text-slate-400">Interactive Hazard &amp; Topology Impact</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {intel?.scenario?.active && (
          <div className="bg-blue-950/30 border border-blue-900/50 p-4 rounded-xl flex items-start justify-between">
            ...
            <button onClick={handleReset} className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg flex items-center gap-2 text-xs font-semibold transition-colors">
        ...
        {/* Heavy Rainfall */}
        <div className="border border-slate-800 rounded-xl p-4 bg-slate-950 shadow-md">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs text-slate-400 uppercase font-bold mb-2 block">Precipitation Surge</label>
              <div className="grid grid-cols-4 gap-2">
                {[1.5, 2, 2.5, 3].map(mult => (
                  <button 
                    key={mult}
                    onClick={() => handleActivate('Heavy Rainfall', { rainfallMultiplier: mult, duration: intel?.scenario?.duration || 24 })}
        ...
        <div className="border border-slate-800 rounded-xl p-4 bg-slate-950 shadow-md">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[ {label: 'Elevated (1.2x)', m: 1.2}, {label: 'High (1.5x)', m: 1.5}, {label: 'Saturated (2.0x)', m: 2.0} ].map(opt => (
              <button 
                key={opt.label}
          ...
          <div className="space-y-3">
            <div>
              ...
              <select 
                ...
                value={intel?.scenario?.active && intel?.scenario?.type === 'Road Blockage' ? failedIds[0] || '' : ''}
              >
                <option value="">Select a corridor to block...</option>
                {safeEdges.filter((e: any) => e.type === 'road').map((e: any) => (
                  <option key={e.id} value={e.id}>Route {e.id} ({e.source} → {e.target})</option>
                ))}
              </select>
            </div>
            
            <div>
              <label className="text-xs text-slate-400 uppercase font-bold mb-2 block">Bridge Structural Failure</label>
              <select 
                className="w-full bg-slate-900 border border-slate-700 rounded-lg p-2 text-sm text-slate-200 outline-none focus:border-blue-500"
                onChange={(e) => {
                  if (e.target.value) handleActivate('Bridge Failure', { failedInfrastructureIds: [e.target.value] });
                }}
                value={intel?.scenario?.active && intel?.scenario?.type === 'Bridge Failure' ? failedIds[0] || '' : ''}
              >
                <option value="">Select a bridge node to fail...</option>
                {safeNodes.filter((n: any) => n.type === 'bridge').map((n: any) => (
